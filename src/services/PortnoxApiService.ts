...
export class PortnoxApiService {
  ...
  static async createSite(site: { name: string; description?: string }, opts?: { projectId?: string; credentialId?: string; directToken?: string; baseUrl?: string }) {
    const { data, error } = await supabase.functions.invoke("portnox-api", {
  ...
  static async createNAS(nas: { name: string; ip?: string; description?: string }, opts?: { projectId?: string; credentialId?: string; directToken?: string; baseUrl?: string }) {
    const { data, error } = await supabase.functions.invoke("portnox-api", {
