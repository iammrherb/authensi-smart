...
const IntelligentScoping = () => {
  ...
  return (
    <PageLayout
      title="Intelligent NAC Scoping Wizard"
      description="Let our AI analyze your environment and create a personalized deployment plan. Get expert recommendations, timeline estimates, and risk assessments in minutes."
      icon={<Brain className="h-6 w-6 text-primary-foreground animate-pulse" />}
    >
      <div className="space-y-8">
        {/* Features Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, index) => (
            <Card key={index} className="text-center hover:shadow-lg transition-shadow gradient-border">
              <CardHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {savedSessions.slice(0, 3).map((session, index) => (
                <Card key={session.id} className="hover:shadow-lg transition-shadow cursor-pointer gradient-border">
                  <CardHeader>
