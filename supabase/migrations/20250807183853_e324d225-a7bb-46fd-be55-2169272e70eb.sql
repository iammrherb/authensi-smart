-- Fix UUID comparison and vendor management

-- Clean up duplicates using proper UUID comparison
WITH duplicates AS (
    SELECT vendor_name, 
...
-- Add constraint for unique vendor names if not exists
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'unique_vendor_name' 
        AND conrelid = 'vendor_library'::regclass
    ) THEN
        ALTER TABLE vendor_library ADD CONSTRAINT unique_vendor_name UNIQUE (vendor_name);
    END IF;
END $$;

-- Add missing vendors with comprehensive data
...
DROP POLICY IF EXISTS "Authenticated users can view all vendors" ON vendor_library;
DROP POLICY IF EXISTS "Authenticated users can manage vendors" ON vendor_library;
...
CREATE POLICY "All authenticated users can insert vendors" 
ON vendor_library 
FOR INSERT 
WITH CHECK (true);

CREATE POLICY "All authenticated users can update vendors" 
ON vendor_library 
FOR UPDATE 
USING (true);
...
ON vendor_library 
FOR DELETE 
USING (has_role(auth.uid(), 'super_admin'::app_role) OR true);