...
! Step 3: Configure VLANs
vlan {{EMPLOYEE_VLAN}} name "{{EMPLOYEE_VLAN_NAME}}" enable
vlan {{GUEST_VLAN}} name "{{GUEST_VLAN_NAME}}" enable
vlan {{IOT_VLAN}} name "{{IOT_VLAN_NAME}}" enable
vlan {{QUARANTINE_VLAN}} name "{{QUARANTINE_VLAN_NAME}}" enable
vlan {{VOICE_VLAN}} name "{{VOICE_VLAN_NAME}}" enable
vlan {{DEFAULT_VLAN}} name "{{DEFAULT_VLAN_NAME}}" enable
...
Networks:
  ...
  - Name: "{{IOT_NETWORK_NAME}}"
    VLAN_ID: {{IOT_VLAN}}
    Subnet: "{{IOT_SUBNET}}"
    DHCP_Mode: "DHCP Server"
    Device_Isolation: true
    
  - Name: "{{QUARANTINE_NETWORK_NAME}}"
    VLAN_ID: {{QUARANTINE_VLAN}}
    Subnet: "{{QUARANTINE_SUBNET}}"
    DHCP_Mode: "DHCP Server"
    
  - Name: "{{VOICE_NETWORK_NAME}}"
    VLAN_ID: {{VOICE_VLAN}}
    Subnet: "{{VOICE_SUBNET}}"
    DHCP_Mode: "DHCP Server"
...
Port_Profiles:
  Portnox_802.1X_Profile:
    ...
    Native_Network: "{{QUARANTINE_NETWORK_NAME}}"
    Voice_Network: "{{VOICE_NETWORK_NAME}}"
    802.1X_Control: "Force Authorized -> Auto"
    MAC_Auth_Bypass: true
    Guest_Network: "{{GUEST_NETWORK_NAME}}"
    Auth_Network: "{{EMPLOYEE_NETWORK_NAME}}"
...
Advanced_Features:
  ...
  DHCP_Snooping:
    Enabled: true
    Networks: ["{{EMPLOYEE_NETWORK_NAME}}", "{{GUEST_NETWORK_NAME}}", "{{IOT_NETWORK_NAME}}"]
...
-- Update vendor library to include missing vendors
INSERT INTO vendor_library (vendor_name, category, vendor_type, support_level, description) VALUES
('Dell Technologies', 'Enterprise', 'Infrastructure', 'tier1', 'Enterprise networking solutions including switches, routers, and wireless systems')
ON CONFLICT (vendor_name) DO NOTHING;

INSERT INTO vendor_library (vendor_name, category, vendor_type, support_level, description) VALUES
('Huawei', 'Enterprise', 'Infrastructure', 'tier1', 'Global provider of ICT infrastructure and smart devices')
ON CONFLICT (vendor_name) DO NOTHING;

INSERT INTO vendor_library (vendor_name, category, vendor_type, support_level, description) VALUES
('Alcatel-Lucent Enterprise', 'Enterprise', 'Infrastructure', 'tier1', 'Enterprise networking and communications solutions')
ON CONFLICT (vendor_name) DO NOTHING;

INSERT INTO vendor_library (vendor_name, category, vendor_type, support_level, description) VALUES
('Ubiquiti', 'Enterprise', 'Infrastructure', 'tier2', 'Enterprise WiFi and networking equipment with centralized management')
ON CONFLICT (vendor_name) DO NOTHING;