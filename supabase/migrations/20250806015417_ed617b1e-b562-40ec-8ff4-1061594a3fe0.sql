-- Fix user management issues with corrected syntax
...
DROP POLICY IF EXISTS "Only functions can insert reset tokens" ON public.password_reset_tokens;
...
DROP POLICY IF EXISTS "Admins can view all vendor_configs" ON public.vendor_configs;
...
DROP POLICY IF EXISTS "Admins can manage vendor_configs" ON public.vendor_configs;
CREATE POLICY "Admins can manage vendor_configs" ON public.vendor_configs
  FOR ALL USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can view all config_templates" ON public.config_templates;
CREATE POLICY "Admins can view all config_templates" ON public.config_templates
  FOR SELECT USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can manage config_templates" ON public.config_templates;
CREATE POLICY "Admins can manage config_templates" ON public.config_templates
  FOR ALL USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can view all template_categories" ON public.template_categories;
CREATE POLICY "Admins can view all template_categories" ON public.template_categories
  FOR SELECT USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can manage template_categories" ON public.template_categories;
CREATE POLICY "Admins can manage template_categories" ON public.template_categories
  FOR ALL USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can view all vendor_compatibility" ON public.vendor_compatibility;
CREATE POLICY "Admins can view all vendor_compatibility" ON public.vendor_compatibility
  FOR SELECT USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can manage vendor_compatibility" ON public.vendor_compatibility;
CREATE POLICY "Admins can manage vendor_compatibility" ON public.vendor_compatibility
  FOR ALL USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can view all firmware_compatibility" ON public.firmware_compatibility;
CREATE POLICY "Admins can view all firmware_compatibility" ON public.firmware_compatibility
  FOR SELECT USING (has_role(auth.uid(), 'super_admin'::app_role));

DROP POLICY IF EXISTS "Admins can manage firmware_compatibility" ON public.firmware_compatibility;
