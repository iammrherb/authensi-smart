...
INSERT INTO vendor_library (name, category, support_level, integration_status, documentation_url, is_verified, vendor_specific_features, capabilities, created_by) VALUES
...
-- Add AI optimization capabilities to configuration templates
UPDATE configuration_templates 
SET ai_optimization_rules = jsonb_build_object(
  'optimization_triggers', array['performance_degradation', 'security_alert', 'compliance_violation', 'capacity_threshold'],
  'ai_models', array['performance_optimizer', 'security_analyzer', 'compliance_checker'],
  'auto_apply', false,
  'confidence_threshold', 0.85,
  'review_required', true
),
template_dependencies = jsonb_build_array(
  jsonb_build_object('type', 'vendor', 'name', 'Aruba Networks', 'version', 'latest'),
  jsonb_build_object('type', 'authentication', 'method', '802.1X', 'required', true)
),
performance_metrics = jsonb_build_object(
  'authentication_time', '< 3 seconds',
  'throughput', '> 1000 authentications/minute',
  'availability', '99.9%',
  'resource_utilization', '< 80%'
)
WHERE configuration_type IN ('authentication', 'network_access', 'security');

-- Update existing vendors with AI-editable fields
UPDATE vendor_library 
SET vendor_specific_features = vendor_specific_features || jsonb_build_object(
...
WHERE name IN ('Cisco', 'Aruba Networks', 'Microsoft', 'Fortinet', 'Palo Alto Networks');