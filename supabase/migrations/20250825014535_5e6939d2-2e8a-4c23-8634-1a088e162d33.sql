-- Fix critical security vulnerabilities in existing library tables
...
ALTER TABLE test_case_library ENABLE ROW LEVEL SECURITY;
...
-- Create restrictive RLS policies for test_case_library
CREATE POLICY "Admin users can view test case library"
ON test_case_library FOR SELECT
TO authenticated
USING (has_role(auth.uid(), 'super_admin'::app_role) OR has_role(auth.uid(), 'product_manager'::app_role));

CREATE POLICY "Admin users can manage test case library"
